...
typedef struct {
        uint32_t freq;
        uint32_t bandwidth;
        radio_lora_spreading_factor_t spreading_factor;
        radio_lora_coding_rate_t coding_rate;
        int8_t power_config;
        bool fix_len;
        bool crc_on;
        bool invert_iq;
...
struct Radio_s
{
     ...
     */
    bool    ( *init )( radio_events_t *events, radio_config_t *config);
    /*
     * brief Re-initialize the radio
     */
    void (*reinit) (void);
    /*!
     ...
     */
    bool    ( *is_channel_free )( RadioModems_t modem, uint32_t freq, int16_t rssiThresh, uint32_t maxCarrierSenseTime );
    /*!
     * \brief Generates a 32 bits random value based on the RSSI readings
     *
     * \remark This function sets the radio in LoRa modem mode and disables
     *	       all interrupts.
     *	       After calling this function either Radio.set_rx_config or
     *	       Radio.set_tx_config functions must be called.
     *
     ...
     */
    void    ( *set_tx_config )( RadioModems_t modem, int8_t power, uint32_t fdev,
     ...
     *
     * \Remark Can only be called once SetRxConfig or set_rx_config have been called
     *
     * \param [IN] modem      Radio modem to be used [0: FSK, 1: LoRa]
     ...
     */
    uint32_t  ( *time_on_air )( RadioModems_t modem, uint8_t pktLen );
    /*!
     ...
     */
    void    ( *start_rx )( uint32_t timeout );
    /*!
     ...
     */
    void    ( *write_buffer )( uint16_t addr, uint8_t *buffer, uint8_t size );
    /*!
     ...
     */
    void    ( *read_buffer )( uint16_t addr, uint8_t *buffer, uint8_t size );
    /*!
     ...
     *			   [0: continuous, others timeout]
     */
    void    ( *rx_boosted )( uint32_t timeout );
    /*!
     ...
     */
    void ( *set_rx_duty_cycle ) ( uint32_t rxTime, uint32_t sleepTime );
};
 ...
 * \brief Computes the packet time on air in ms for the given payload
 *
 * \Remark Can only be called once set_rx_config or set_tx_config have been called
 *
 * \param [IN] modem	  Radio modem to be used [0: FSK, 1: LoRa]
 * \param [IN] pktLen	  Packet payload length
 *
