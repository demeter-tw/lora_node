...
int fw_env_write(char *name, char *value)
{
        ...
        for (nxt = env = envir.data; *env; env = nxt + 1) {
                for (nxt = env; *nxt; ++nxt) {
                        if (nxt >= &envir.data[CONFIG_FW_ENV_SIZE]) {
                                ...
                                return NCORE_ERR_FW_ENV_NOT_TERMINATED;
                        }
                }
	 ...
	 */
        ...
        if (len > (&envir.data[CONFIG_FW_ENV_SIZE] - env)) {
                ...
                return NCORE_ERR_FW_ENV_OVERFLOW;
        }
...
finish:
        ...
        *envir.crc = crc_calc((uint32_t *) envir.data, (CONFIG_FW_ENV_SIZE - sizeof(envir.crc)) / sizeof(uint32_t));
        ...
        internal_flash_erase(CONFIG_FW_ENV_START_ADDR, 1);
        flash_setting_addr = CONFIG_FW_ENV_START_ADDR;
        internal_flash_write(&flash_setting_addr, (uint32_t *) envir.image, CONFIG_FW_ENV_SIZE / sizeof(uint32_t));
        internal_flash_lock();
#endif
...
void fw_env_save(void)
{
        ...
        *envir.crc = crc_calc((uint32_t *) envir.data, (CONFIG_FW_ENV_SIZE - sizeof(envir.crc)) / sizeof(uint32_t));
        ...
        internal_flash_write(&flash_env_addr, (uint32_t *) envir.image, CONFIG_FW_ENV_SIZE / sizeof(uint32_t));
        internal_flash_lock();
}
...
fail:
        ...
        return NCORE_ERR_FW_ENV_STRING_SPLIT;
}
...
int fw_env_open(void)
{
        ...
        internal_flash_read(&flash_start_addr, envir.image, CONFIG_FW_ENV_SIZE / 4);

        crc = crc_calc((uint32_t *) envir.data, (CONFIG_FW_ENV_SIZE - sizeof(envir.crc)) / sizeof(uint32_t));
        ...
        if (!crc_ok) {
                ...
                *envir.crc = crc_calc((uint32_t *) envir.data, (CONFIG_FW_ENV_SIZE - sizeof(envir.crc)) / sizeof(uint32_t));
                ...
                internal_flash_erase(CONFIG_FW_ENV_START_ADDR, 1);
                flash_start_addr = CONFIG_FW_ENV_START_ADDR;
                internal_flash_write(&flash_start_addr, (uint32_t *) envir.image, CONFIG_FW_ENV_SIZE / sizeof(uint32_t));
                internal_flash_lock();
        }
